...
export default function TodoList({ boardId, indexCard, indexList }) {
  ...
  const a = <div>{ todoList ? todoList.map((todo, index) => <Todo todo={ todo } key={ index } indexCard={ indexCard } indexList={ indexList } boardId={ boardId } />) : '' }</div>
  const b = <div>{ todoListDos ? todoListDos.map((todo, index) => <Todo todo={ todo } key={ index } indexCard={ indexCard } indexList={ indexList } boardId={ boardId } />) : '' }</div>
              ...
              <Grid container>
                <Grid item xs={ 12 } sm={ 12 }>
                  <Grid container>
                    ...
                    <Grid item md={ 11 } style={ { width: '100%' } }>
                      { Number.isNaN(x) === false ?
                        <>
                          <div className={ classes.listcv }>
                            <div style={ { textAlign: 'center', margin: 'auto' } } type="text">
                              <Button onClick={ handleToggle } type="number" style={ { "width": "100%", "margin": "0px 0px 0px 110px" } }>
                                { isActive === true ? `Hiện những mục đã xong (${PercentDone})` : "Ẩn các mục đã hoàn thành" }
                              </Button>

                              <input type="range" value={ x } style={ { width: '165%' } } onChange={ zzz }></input>
