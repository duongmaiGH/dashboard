...
function TrelloCard({ text, id, index, listID, title, boardId, indexList }, props) {
    ...
    useEffect(() => {
        const listRef = myFirebase.database().ref('cardMembers/').orderByChild('cardId').equalTo(id)
        listRef.on('value', snapshot => {
            const lists = snapshot.val();
            const memberList = []
            for (let id in lists) {
        ...
        myFirebase.database().ref('/emailToUid/' + emailWithoutDot).once('value', function (snapshot) {
            if (snapshot.exists()) {
                ...
                myFirebase.database().ref('/userBoards/' + userId).once('value', function (snap) {
                    ...
                    if (snap.exists()) {
                        myFirebase.database().ref('/cardMembers/' + id).child('name').once('value', function (childsnap) {
                            if (childsnap.exists()) {
                                alert('Người dùng đã được gán vào card!')
                            }
                            else {
                                if (email.length > 0 && email.length < 50) {
                                    myFirebase.database().ref('/cardMembers').child(id).set({
                                        cardId: id,
                                        name: email
                                    })
                                }
                                                            ...
                                                            { memberList ? memberList.map((member, i) =>
                                                                ...
                                                                <div style={ { marginTop: 20, } }>
                                                                    <div className={ classes.member } key={ i }>
                                                                        <span className={ classes.listmember }>
                                                                            { member.name[ 0 ] }
                                                                        </span>
                                                                    </div>
                                                        ...
                                                        <Grid item md={ 11 } >
                                                            ...
                                                            { fileList ?
                                                                fileList.map((file, i) =>
                                                                    <>
                                                                        <div key={ i }>
                                                                            <a className={ classes.link } style={ { float: 'left' } } href={ file.url }
                                                                            ...
                                                                            <Button onClick={ onDeleteFile } value={ file.id }>Delete</Button>
                                                                        </div>
                                                                        <br />
                                                                        <hr />
                                                                    </>
